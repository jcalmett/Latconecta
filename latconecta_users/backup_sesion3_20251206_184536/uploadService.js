...
const uploadService = {
   ...
   * @param {string} category - Categoría (products, services, companies, users, countries)
  ...
  uploadImage: async (file, category = 'users') => {
    try {
      ...
      const response = await fetch(`${baseURL}/upload/image?category=${category}`, {
        method: 'POST',
        body: formData,
        headers: {
  ...
  deleteImage: async (fileUrl) => {
    try {
      ...
      const data = await apiClient.delete('/upload/image', {
        params: { file_url: relativePath }
      });
